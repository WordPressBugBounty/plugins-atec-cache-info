...
(function() {
	...
	if (is_null( $licenseOk = TOOLS::page_header($una,2) )) return;
		...
		switch ($una->nav)
		{
			...
			case 'PHP_'.__('Extensions', 'atec-cache-info'):
				if (TOOLS::pro_feature($una, '„Extension“ lists all active PHP extensions and checks whether recommended extensions are installed', false, $licenseOk))
			...
			default:
				...
				{
					if (TOOLS::pro_feature($una, '„OPC Scripts“ lists all scripts files and statistics of in the OPcache memory', false, $licenseOk))
